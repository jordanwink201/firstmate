...
`fm_backend_cmux_send_readiness` reuses both signals for `fm-send.sh`'s text-send preflight: the shared busy-footer regex over a bounded capture tail first (cmux has no native agent-state primitive), then this structural composer check - `empty` maps to `ready`, `pending` stays `pending`, and an unreadable surface (including the fresh-surface `read-screen` failure) stays a conservative `unknown`.
