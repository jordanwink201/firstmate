...
You do not need to attach for routine supervision: from an active firstmate session, `bin/fm-peek.sh fm-<id>` reads a task's pane without attaching, and `FM_HOME=<this-firstmate-home> bin/fm-send.sh fm-<id> --key <key>` steers keys unless `FM_HOME` is already set to the active firstmate home.
Text sends through `fm-send.sh` currently refuse before typing: zellij has no busy/composer readiness probe yet, so `fm_backend_send_readiness` (`bin/fm-backend.sh`) reports a conservative `unknown` for a live zellij target and `fm-send.sh` requires an affirmative `ready` (see "Known gaps left for a follow-up").
...
Limitations: zellij is experimental, has no per-home workspace split (all tasks share one tab bar, unlike herdr), has no verified agent-process liveness classifier for the session-start secondmate sweep, and still carries the known gaps documented below (no native busy-state signal, conservative send readiness that refuses `fm-send.sh` text sends, and a narrow focus-steal race on tab creation) - see "Known gaps left for a follow-up" at the end of this document.
...
- **No send-readiness probe, so `fm-send.sh` text sends refuse.** `fm_backend_send_readiness` (`bin/fm-backend.sh`) reports a conservative `unknown` for a live zellij target (`missing` when gone), and `fm-send.sh` requires an affirmative `ready` before typing; only `--key` sends pass. Closing this needs a zellij busy check plus a composer-state read the delta-based submit verifier does not provide. Pinned by `tests/fm-send-readiness.test.sh`.
