...
The Kimi installer requires an existing regular non-symlink `~/.kimi-code/config.toml`, `python3` with `tomllib` or `tomli`, and `jq`; it validates but never serializes the captain's TOML and refuses before writing when the config is missing, malformed, or surprising or when either tool requirement is unavailable.
  ...
  "rules": [
    {
      ...
      "select": "<optional legacy selector; only quota-balanced is accepted>",
...
Rule `select` is optional and only accepts `quota-balanced`.
That field is legacy-compatible spelling for the current implicit quota-aware array behavior; it has no effect on single-object `use`.
...
Malformed JSON, an empty or malformed rule/default array, an unknown selector, an unverified harness, or an effort value unsupported by that harness is reported as `CREW_DISPATCH: invalid config/crew-dispatch.json - ...`; missing `jq` is reported through the normal `MISSING: jq` install-consent flow.
...
The helper reads unformatted MCP inventory through the existing AXI session so titled pages retain their URLs and selected markers with the pinned transport.
...
FM_CAPTAIN_RE='done:|needs-decision:|needs-retier:|blocked:|failed:|PR ready|checks green|ready in branch|merged'   # captain-relevant status regex; nonterminal progress verbs remain excluded even when their prose matches
FM_CLASSIFY_PAUSED_VERB=paused     # leading status verb for a declared external wait; excluded from FM_CAPTAIN_RE and distinct from blocked
FM_FIRST_PROGRESS_SECS=480         # seconds after spawn_ts before an ordinary crewmate with no status line and no progress evidence surfaces as stuck-at-launch
...
FM_HEARTBEAT_SCAN_SECS=300         # cadence of the catch-all scan for missed captain verbs and launch stalls
