...
Before typing a text send, `fm-send.sh` requires `fm_backend_send_readiness` (`bin/fm-backend.sh`) to report affirmatively `ready` from its small stable vocabulary (`ready`/`busy`/`pending`/`unknown`/`missing`): a gone endpoint is `missing` for every backend, tmux/herdr/cmux prove a non-busy target with a structurally empty composer, orca and zellij stay a conservative `unknown` (so their text sends currently refuse), and `--key` sends bypass the composer preflight.
...
It owns task endpoint creation, bounded capture, text/key sends, send-readiness classification for text sends, current-path reads for spawn-time worktree discovery when the backend does not create the worktree itself, live-window fallback lookup, agent-process liveness probes where verified, and endpoint teardown.
