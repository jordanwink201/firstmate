...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or an X-mode mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, launch stalls past `FM_FIRST_PROGRESS_SECS`, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
A launched ship/scout task with `spawn_ts=`, no nonblank status line, no dirty files or branch commits, and for scouts no useful `data/<id>/report.md` update after `FM_FIRST_PROGRESS_SECS` is `stuck-at-launch`; secondmate idle panes are excluded.
The shared classifier treats missing or ambiguous launch metadata, Git checks, and scout-report checks as non-actionable rather than guessing.
...
`bin/fm-fleet-view.sh` renders that snapshot as Markdown for humans, while `bin/fm-bearings-snapshot.sh` provides bounded bearings and firstmate-internal advisory projections, so all read-only views consume one structured contract instead of reparsing raw fleet files.
The script header owns the exact projection schemas.
...
The default bearings path remains local-only; live GitHub enrichment exists only behind the bearings `--include-prs` opt-in, and the internal advisory path is always local-only.
...
The watcher and daemon share `bin/fm-classify-lib.sh` for captain-relevant status verbs, declared-external-wait vocabulary, launch-watchdog predicates, and status-scan primitives.
...
The daemon escalates captain-relevant events, launch stalls, plus a bounded recheck for a declared pause that remains idle, as one batched, single-line digest using the canonical `away-supervisor` kind from `bin/fm-operational-input.sh` so firstmate can distinguish it structurally from real messages.
