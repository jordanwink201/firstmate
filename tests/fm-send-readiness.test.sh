...
assert_text_send_blocks_before_literal() {  # <mode> <expected-state>
  local mode=$1 expected=$2 home send_log tmux_log out rc
  home=$(setup_home "block-$mode")
  send_log="$home/send.log"; tmux_log="$home/tmux.log"
  out=$(run_send_with_mode "$mode" "$home" "$send_log" "$tmux_log" "sess:win" "do not type" 2>&1)
  rc=$?
...
test_key_path_bypasses_readiness() {
  ...
  send_log="$home/send.log"; tmux_log="$home/tmux.log"
  out=$(run_send_with_mode missing "$home" "$send_log" "$tmux_log" "sess:win" --key Enter 2>&1)
  rc=$?
  expect_code 0 "$rc" "--key path should bypass readiness"$'\n'"$out"
  [ ! -s "$send_log" ] || fail "--key path should not type literal text"
  assert_not_contains "$(cat "$tmux_log")" "display-message" "--key path should not run readiness target_exists"
