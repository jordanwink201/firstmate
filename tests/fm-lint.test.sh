...
test_source_graph_boundaries_keep_every_owner() {
  ...
  [ "$production_context_tests" = 'fm-backend-herdr.test.sh|fm-backlog-handoff.test.sh|fm-daemon.test.sh|fm-pending-reply.test.sh|fm-secondmate-lifecycle-e2e.test.sh|fm-secondmate-sync.test.sh|fm-send-readiness.test.sh|' ] \
    || fail "only approved source-aware interop tests may retain production source context: $production_context_tests"
