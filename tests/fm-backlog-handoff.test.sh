...
# (the same probe the handoff itself gates on), matching the backend smoke suites.
...
fm_tasks_axi_compatible || { echo "skip: tasks-axi is older than 0.2.2 (atomic multi-ID mv required by the delegated handoff path)"; exit 0; }
