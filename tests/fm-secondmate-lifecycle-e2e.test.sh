...
phase_handoff() {
  ...
  # too old for atomic multi-ID mv - the same probe the handoff itself gates on
  # (the downstream recovery and teardown phases do not depend on this phase).
  ...
  if ! fm_tasks_axi_compatible; then
    echo "skip: tasks-axi is older than 0.2.2 (atomic multi-ID mv required by the handoff phase)"
    return 0
  fi
