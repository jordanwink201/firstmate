...
For every registered secondmate, validated readable structured facts from its own home are
...
MODEL=$(printf '%s' "$SNAP" | jq \
  ...
  def round_robin_landed($n):
    ...
    | [range(0; (($groups | map(length) | max) // 0)) as $i
       ...
       | select(length > $i)
       | .[$i]][:$n];
  def queued_captain_actionable:
    (.captain_actionable == true)
    or (((.kind // "") == "captain" or (.hold_kind // "") == "captain")
  ...
  | ([ (.secondmate_current.records // [])[]
       | ([.decisions_open[]? | select(.source == "backlog" and .verb == "captain-hold")]) as $native_captain_holds
       | ($native_captain_holds
          + [ .queued[]?
              | select(queued_captain_actionable)
              | select(.id as $queued_id | [$native_captain_holds[].id] | index($queued_id) | not)
              | {id,key:.id,verb:"captain-hold",summary:(.title // .id),
                 reason:(.hold_reason // .blocked_reason // "captain decision pending"),source:"backlog"} ]) as $captain_holds
       | ([.holds[]? | select(.source == "backlog")]) as $backlog_holds
       | . + {
           bearings_captain_holds:$captain_holds,
           bearings_holds:(if .current.state == "captain_decision" or ($captain_holds | length) > 0 then $backlog_holds else .holds end),
           bearings_state:(
             if .current.state == "unknown" then "unknown"
             elif ($captain_holds | length) > 0 then "captain_decision"
             elif .current.state == "captain_decision" then
               if (.active_children | length) > 0 then "active_child_work"
  ...
  | ((if (.main_inventory.valid == false) then
     ...
     + [ .backlog.records[]
         ...
         | select(queued_captain_actionable | not)
         | select(($all_queued == 1)
