...
#   It is advisory attribution: mismatches warn and record an override_* reason, never
#   substituting a profile or blocking an explicit captain override.
...
#   source of truth; shared --scout/--harness/--model/--effort/--rule/--backend applies to every pair.
...
dispatch_rule_profile_matches() {
  local rule=$1 actual_harness=$2 actual_model=$3 actual_effort=$4 file idx
  ...
  jq -e \
    --argjson idx "$idx" \
    --arg h "$actual_harness" \
    --arg m "${actual_model:-default}" \
    --arg e "${actual_effort:-default}" '
      (.rules // [])[$idx].use? as $use
      | if ($use | type) == "array" then $use else [$use] end
      | map(select(. != null))
      | any(. as $p
          | ($p.harness // "") == $h
          and ($p.model // "default") == $m
          and ($p.effort // "default") == $e)
    ' "$file" >/dev/null 2>&1
}

dispatch_rule_expected_label() {
  ...
  jq -er --argjson idx "$idx" '
    ...
    | if ($use | type) == "array" then $use else [$use] end
    | map(select(. != null))
    | map((.harness // "") + "/" + (.model // "default") + "/" + (.effort // "default"))
    | join(",")
  ' "$file" 2>/dev/null
}
...
resolve_dispatch_rule_meta() {
  ...
  elif dispatch_rule_profile_matches "$RULE" "$HARNESS" "${MODEL:-default}" "${EFFORT:-default}"; then
    ROUTING_RULE_META=$RULE
  else
    expected=$(dispatch_rule_expected_label "$RULE" || true)
    actual=$(dispatch_profile_label "$HARNESS" "${MODEL:-default}" "${EFFORT:-default}")
    if [ -n "$expected" ]; then
  ...
  if [ "$KIND" = ship ] && [ "$HARNESS" = claude ]; then
    case "$ROUTING_RULE_META" in
      override*|'')
        echo "warning: ship on claude without a matched dispatch rule (rule=${ROUTING_RULE_META:-unset}); confirm intended" >&2 ;;
    esac
  fi
}
