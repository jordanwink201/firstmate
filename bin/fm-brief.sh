...
4. For browser QA, use \`$FM_ROOT/bin/fm-browser-qa.sh --url <exact-url> --out <evidence-dir> --start-if-needed\`. The exact QA URL must come from this brief, a linked task/PR, or firstmate; if it is unclear, append \`needs-decision: ask the captain for the exact QA URL\` and stop. Do not improvise Python websocket, chrome-remote-interface, Playwright, or other browser stacks unless this project already owns that tooling. Treat helper failures that start with \`blocked:\` as your own \`blocked:\` status and stop; when auth is expired, the helper foregrounds the QA Chrome profile for human login. Keep the helper's \`identity.json\`, \`snapshot.txt\`, \`screenshot.png\`, and \`report.md\` evidence with your validation notes. Before follow-up \`chrome-devtools-axi\` work in a separate AXI session, run \`$FM_ROOT/bin/fm-browser-qa.sh --select-identity <evidence-dir>/identity.json --axi-session <session> --out <evidence-dir>/attach\`; use that same \`CHROME_DEVTOOLS_AXI_SESSION\` and the identity's \`browser_url\` for later AXI commands, and never reuse \`identity.json.page_id\` directly across sessions.
5. Report status by appending one line:
   \`echo "{state}: {one short line}" >> $STATUS_FILE\`
   States: working, needs-retier, needs-decision, blocked, $PAUSED_VERB, done, failed.
   Each append wakes firstmate, so report sparingly: only phase changes a supervisor
   would act on and the needs-retier/needs-decision/blocked/$PAUSED_VERB/done/failed states. No step-by-step
...
8. If the investigation spans more than two subsystems or you cannot reach a confident root cause,
