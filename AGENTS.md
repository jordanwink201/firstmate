...
config/git-author  optional crewmate commit identity; LOCAL, gitignored; name=... and email=... pin worktree Git config plus GIT_AUTHOR_*/GIT_COMMITTER_* for spawned agents and inherited secondmate homes; without this file, spawn requires a complete global Git identity
...
For browser QA, use `bin/fm-browser-qa.sh` with an exact known URL and evidence directory; relay its `blocked:` results rather than improvising a browser stack.
Tracked GitHub guard hooks enforce this fork's origin/upstream mutation boundary; never push or PR to upstream when working from a maintained fork lane.
...
When a configured rule matched, pass `--rule <n>` for lightweight provenance; explicit captain overrides still win and may record override_* provenance in task metadata.
...
Before reporting a no-mistakes registration or run failure, have the worker capture the final branch and `git rev-parse --short HEAD`.
Then confirm the no-mistakes `current_branch` and run `head` match the final branch and commit before treating the failure as actionable.
...
2. For `stale:`, inspect the recorded endpoint and load `stuck-crewmate-recovery` for a stopped, looping, confused, or unresponsive worker; a `stuck-at-launch` reason means the worker passed `FM_FIRST_PROGRESS_SECS` without first-progress evidence, and a deep-inspection reason also requires current-state and validation-log inspection.
...
Scout briefs carry the `needs-retier` ratchet when a bounded investigation exceeds its tier; treat that status as a request to respawn the work on a higher-tier profile.
