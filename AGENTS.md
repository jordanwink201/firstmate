...
For browser QA, use `bin/fm-browser-qa.sh` instead of improvising a browser stack: pass the exact task/preview URL with `--url <exact-url>` and an evidence directory with `--out <dir>`; the helper matches only the browser-normalized form of that URL (such as the trailing slash Chrome adds to a bare origin), never fuzzy variants.
