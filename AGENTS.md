...
A text send also refuses before typing unless the backend's send readiness is affirmatively ready - a busy turn, pending composer input, or an unprovable state all exit non-zero (`fm_backend_send_readiness` in `bin/fm-backend.sh`); `--key` sends bypass that composer preflight.
