...
The shared classifier also owns the first-progress launch watchdog: a `stuck-at-launch` stale or heartbeat wake means an ordinary crewmate passed `FM_FIRST_PROGRESS_SECS` with no status line and no Git progress, so handle it like a stale crewmate that stopped without reporting.
