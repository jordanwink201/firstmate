...
The same launch and the same locked bootstrap sweep also propagate the primary's declared inherited local material: `config/crew-dispatch.json`, `config/crew-harness`, `config/git-author`, `config/backlog-backend`, `config/herdr-presentation-spaces`, and the one shared captain-preference file `data/captain-shared.md`.
...
Propagation failures warn without blocking secondmate launch or session-start continuation, except `config/git-author` that cannot converge blocks secondmate launch and makes focused config push fail non-zero when the primary source exists or a stale downstream copy must be removed.
Session-start bootstrap reports that failure as an inheritance failure for operator action while leaving the running secondmate untouched.
