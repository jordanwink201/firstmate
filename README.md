...
- **Read-only fleet dashboard** - an optional localhost-only server (`bin/fm-dashboard-server.sh`) renders live fleet state from cached snapshots of a read-only probe, and teardown keeps a local routing-economics ledger summarized by `bin/fm-route-report.sh`; see [docs/scripts.md](docs/scripts.md).
...
- Git with a complete author identity or local [`config/git-author`](docs/configuration.md#crewmate-git-author-identity-configgit-author), and the GitHub CLI authenticated through `gh auth login`.
...
| Skill              | What it does                                                                                                                                  |
| ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `/afk`             | Enter away-mode supervision: the sub-supervisor self-handles routine wakes in bash, escalates captain-relevant events, launch stalls, and bounded declared-external-wait rechecks as batched digests, and actively alerts if delivery wedges while you step away |
